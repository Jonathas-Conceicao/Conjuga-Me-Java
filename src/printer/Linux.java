...
public class Linux{
      ...
      for (j = 0; j < 3 ;j++) {
        ...
        if(this.v.getIndicativo()[j][i].getSecond()){
          System.out.print(this.inregularColor +
                           this.v.getIndicativo()[j][i].getFirst() +
                           this.defaultColor);
        }else{
  ...
  private void printConjuntivo(){
    ...
    for (i = 0; i < 6; i++) {
      for (j = 0; j < 3 ;j++) {
        ...
        offset += this.printWithLength(this.getPerson(i));
        if(this.v.getConjuntivo()[j][i].getSecond()){
          System.out.print(this.inregularColor +
                           this.v.getConjuntivo()[j][i].getFirst() +
                           this.defaultColor);
        }else{
          System.out.print(this.regularColor +
                           this.v.getConjuntivo()[j][i].getFirst() +
        ...
        offset += this.length(this.v.getConjuntivo()[j][i].getFirst());
        printTab(offset);
      }
      System.out.print("\n");
    }
  ...
  private void printImperativo(){
    ...
    if(this.v.getImperativo()[2][0].getSecond()){
      System.out.print(this.inregularColor +
      this.v.getImperativo()[2][0].getFirst() +
      this.defaultColor);
    }else{
      System.out.print(this.regularColor +
      this.v.getImperativo()[2][0].getFirst() +
    ...
    for (i = 1; i < 6; i++) {
      for (j = 0; j < 3 ;j++) {
        ...
        if(this.v.getImperativo()[j][i].getSecond()){
          System.out.print(this.inregularColor +
                           this.v.getImperativo()[j][i].getFirst() +
                           this.defaultColor);
        }else{
          System.out.print(this.regularColor +
                           this.v.getImperativo()[j][i].getFirst() +
        ...
        offset += this.length(this.v.getImperativo()[j][i].getFirst());
        offset += this.printWithLength(this.getPerson(i));
        printTab(offset);
      }
      System.out.print("\n");
